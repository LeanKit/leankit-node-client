...
const parseReplyData = ( error, response, body, callback, cacheCallback ) => {
	if ( error ) {
		...
		return callback( err, body );
	} else if ( body && body.ReplyCode && body.ReplyCode > 399 ) { // eslint-disable-line no-magic-numbers
...
const LeanKitClient = ( ...args ) => { // eslint-disable-line max-statements
	...
	const addCardWithWipOverride = ( boardId, laneId, position, wipOverrideReason, card, callback ) => { // eslint-disable-line max-params
	...
	const moveCard = ( boardId, cardId, toLaneId, position, wipOverrideReason, callback ) => { // eslint-disable-line max-params
		return clientPost( `board/${ boardId }/movecardwithwipoverride/${ cardId }/lane/${ toLaneId }/position/${ position }`, {
			comment: wipOverrideReason
		}, callback );
	};

	const moveCardByExternalId = ( boardId, externalCardId, toLaneId, position, wipOverrideReason, callback ) => { // eslint-disable-line max-params
	...
	const moveTask = ( boardId, cardId, taskId, toLaneId, position, callback ) => { // eslint-disable-line max-params
